...
class Car():
    ...
    def annotate_image(self, undist_image, ret):
        ...
        if ret:
            ...
            if self.dist_from_center < 0:
                offset_text = 'Vehicle is {:.2f} meters left of center'.format(self.dist_from_center)
            elif self.dist_from_center > 0:
                offset_text = 'Vehicle is {:.2f} meters right of center'.format(self.dist_from_center)
            else:
                offset_text = 'Vehicle is in center'.format(self.dist_from_center)
            ...
            warn_text = 'No lane detection!!!'
            ret, baseline = cv2.getTextSize(warn_text, fontFace, fontScale, thickness) 
            x_begin = int(x_shape/2 - ret[0]/2)
            y_begin = ret[1] + baseline
            undist_image = cv2.putText(undist_image, warn_text, (x_begin, y_begin), fontFace, fontScale, (0,0,255), thickness)
        return undist_image
